...
type CCustomUploadProps = {
  form: any;
  name: string;
};

const CCustomUpload: FC<CCustomUploadProps> = ({ form, name }) => {
  ...
  React.useEffect(() => {
    async function getImage() {
      const fileName = `public/${form.getValues(name)}`;

      const { publicUrl } = await supabaseGetPublicUrl(fileName, 'company');
